...
pid_t init_monitor_info(struct travel_monitors *tmonitor_info, int index, char *bufferSize_str, char *input_dir,int hashes,int filter_size,int height);

//travel monitor calls this function to create as many minitors as given by the user
void init_monitors(struct travel_monitors *tmonitor_info, int n, int bufferSize, char *input_dir,int hashes,int filter_size,int height);
