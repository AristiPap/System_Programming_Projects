...
//travelRequest citizenID date countryFrom countryTo virusName
void travelRequest(struct travel_monitors *tmonitor_info, int monitor_size, int bufferSize,char *citizen_id, char *date,char *countryFrom,char *countryTo,char *virus_name, List viruses,List viruses_travel_stats)
{
  ...
  if(found == NULL){
    ...
    for(int i=0; i<monitor_size; i++){
        write_msg_to_pipe(tmonitor_info[i].write_fd, TRAVEL_REQUEST, "no", bufferSize); 
    ...
    for(ListNode current = tmp->dummy->Next; current != NULL; current = current->Next){
      ...
      if(flg_found==true){
        ...
        if(flg == false){
          flg = Search( (*(virusNode *)current->info).blf,citizen_id);
          if(flg==false){
        ...
        //if bloom filter returns a positive answer then we have to go search to the monitors structs
          ...
          for(int i=0; i<monitor_size; i++){
            ...
            //in order to know whether the request will be accepted or rejected we'll have to find out from the minitor
              ...
              write_msg_to_pipe(tmonitor_info[i].write_fd, TRAVEL_REQUEST, msg, bufferSize);
              write_msg_to_pipe(tmonitor_info[i].write_fd, READY, "", bufferSize);  
              free(msg);
              found_monitor = true;
            }
...
void searchVaccinationStatus(struct travel_monitors *tmonitor_info, int monitor_size, int bufferSize,char *citizenID)
{
    for (int i = 0; i < monitor_size; ++i)  
    {
      write_msg_to_pipe(tmonitor_info[i].write_fd, SEARCH_VAC_STAT, citizenID, bufferSize);  // Command to search
      write_msg_to_pipe(tmonitor_info[i].write_fd, READY, "", bufferSize);
    }
