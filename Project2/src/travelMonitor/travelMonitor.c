
#include "../../include/header.h"
#include "../../include/tm_queries.h"
#include "../../include/init_monitors.h"
#include "../../include/signal_handler.h"
#include "../../include/DB_Hashes.h"
#include "../../include/VaccinMonitor_Utilities.h"
#include "../../include/CommandEditing.h"

// Process a user command. If needed, forward the request to the required workers.
// Returns 1 if the command given was /exit, else 0.
int get_command(char *cp,char *line, int ccode, int *available_monitors, int *total, int monitor_size, struct travel_monitors *tmonitor_info, int bufferSize, List B_filters,List viruses_travel_stats,bloom_filter *bf,DIR *input_dir)
{
  //variables that will be used to get the info from the command given by the user
  char *stok_save;
  char *command;
  char ID[5];
  char _vname[30];
  char date1[11];
  char date2[11];
  char cname1[30];
  char cname2[30];
  char name[13];
  char lastname[13];
  int age;
  char action[4];
  strtok_r(line, " \n", &stok_save);  // Initialize strtok_r
  switch(ccode)
  {
    case TRAVEL_STATS:
    // /travelStats virusName date1 date2 [country]
      command = strtok(NULL," ");
      strcpy(_vname,command);
      command = strtok(NULL," ");
      strcpy(date1,command);
      command = strtok(NULL," ");
      strcpy(date2,command);
      
      if(count(cp,' ')==3){
      //then no country given as a parameter
        travelStats(_vname,date1,date2,"None",viruses_travel_stats);
      }
      else{
        command = strtok(NULL," ");
        //strcpy(cname1,command);
        char *cname = malloc((strlen(command)+1)*sizeof(char));
        strcpy(cname,command);
        cname[strlen(command)-1] = '\0';
        travelStats(_vname,date1,date2,cname,viruses_travel_stats);
        free(cname);
      }
      return 0;
    
    case TRAVEL_REQUEST:
      *available_monitors = 0;
      command = strtok(NULL," ");
      strcpy(ID,command);
      command = strtok(NULL," ");
      strcpy(date1,command);
      command = strtok(NULL," ");
      strcpy(cname1,command);
      command = strtok(NULL," ");
      strcpy(cname2,command);
      command = strtok(NULL," ");
      char *vname = malloc((strlen(command)+1)*sizeof(char));
      strcpy(vname,command);
      vname[strlen(command)-1] = '\0';
      travelRequest(tmonitor_info,monitor_size,bufferSize,ID,date1,cname1,cname2,vname,B_filters,viruses_travel_stats);
      free(vname);
      return 0;
      
    case SEARCH_VAC_STAT:
      *available_monitors = 0;
      command = strtok(NULL," ");
      char *_id = malloc((strlen(command)+1)*sizeof(char));
      strcpy(_id,command);
      _id[strlen(command)-1] = '\0';
      searchVaccinationStatus(tmonitor_info,monitor_size,bufferSize,_id);
      free(_id);
      return 0;
    case COUNTRY_CHECK:
      // /addVaccinationRecords country
      *available_monitors = 0;
      //find monitor that handles the given country
      command = strtok(NULL," ");
      char *countryname = malloc((strlen(command)+1)*sizeof(char));
      strcpy(countryname,command);
      countryname[strlen(command)-1] = '\0';
      for(int i=0; i<monitor_size; i++){
        ListNode found = ListFind(tmonitor_info[i].countries,countryname,2);
        if(found!=NULL){
          write_msg_to_pipe(tmonitor_info[i].write_fd,COUNTRY_CHECK,countryname,bufferSize);
          write_msg_to_pipe(tmonitor_info[i].write_fd, READY, "", bufferSize);
        }
        else{
          write_msg_to_pipe(tmonitor_info[i].write_fd,COUNTRY_CHECK,"",bufferSize);
          write_msg_to_pipe(tmonitor_info[i].write_fd, READY, "", bufferSize);
        }
      } 
      free(countryname);
      return 0;
    case EXIT:
      TM_Destructor(tmonitor_info,monitor_size,input_dir,bf,B_filters,viruses_travel_stats);
      return 1;
    case 13:
      help();
      return 0;
    default:
      printf("ERROR: %d: Incorrect menu option, type -h if you want to see the menu option!\n", -1);
  }
}

int get_msg_from_monitor(int opcode, char *sanitised, int *total, int *available_monitors,List viruses,List viruses_travel_stats,bloom_filter *new_bf)
{
  switch(opcode)
  {
    case WORKER_READY:  // Ready check
      ++(*available_monitors);
      return 0;
    
    case INIT_DB:  //Insert in db bloom filters and info after monitor processed its data
      init_db(sanitised,viruses,k_hashes,_bf_size,new_bf);
      return 0;
    
    case UPDATE_DB:  // Update the db with new bloom filters and info after user added new filew in given directory 
      update_db(sanitised,viruses,k_hashes,_bf_size,new_bf);
      return 0;
    
    //monitor that controls the country given in the /addVaccinationRecords command returns message that he is ready.
    case COUNTRY_CHECK_RES:
      puts(sanitised);
      return COUNTRY_CHECK;
      
    case TRAVEL_REQUEST_RES:
      if(strcmp(sanitised,"no")==0)
        return 0;
      bool flg;
      //after we have received a report we check whether its positive/negative for the travel stats query
      char *token = strtok(sanitised,"/");
      char *action = malloc((strlen(token)+1)*sizeof(char));
      strcpy(action,token);
      if(strcmp(action,"YES")==0){
        flg=true;
      }  
      else
        flg=false;
      token = strtok(NULL, "/");
      char *virus_name = malloc((strlen(token)+1)*sizeof(char));
      strcpy(virus_name,token);
      token = strtok(NULL, "/");
      char *date = malloc((strlen(token)+1)*sizeof(char));
      strcpy(date,token);
      token = strtok(NULL, "/");
      char *country_name = malloc((strlen(token)+1)*sizeof(char));
      strcpy(country_name,token);
      
      ListNode find = ListFind(viruses_travel_stats,virus_name,5);
      if(find==NULL){
        virus_travel_stats *vnode = VTravelStats(country_name,virus_name);
        ListPushBack(vnode,viruses_travel_stats,sizeof(*vnode));
        find = ListGetLast(viruses_travel_stats);
        free(vnode);
      }
      Date_Info *dnode = date_init(date,flg);
      List tmp2 = (*(virus_travel_stats *)find->info).travel_stats;
      ListNode findCountry = ListFind(tmp2,country_name,4);
      ListPushBack(dnode,(*(travel_stat_info *)findCountry->info).date_info, sizeof(*dnode));
      //deallocate memory
      free(dnode);
      free(virus_name);
      free(date);
      free(action);
      free(country_name);
      return 0;
    case SEARCH_VAC_STAT_RESULT:
      printf("%s\n",sanitised);
      return 0;
  }
}

int main(int argc, char *argv[])
{
  //signal_handler_init();  // Signals blocked during a command process
  signals_block();   // Block signals during setup
  if(argc!=9){
      printf("You entered wrong input.You have to type:./travelMonitor -m numMonitors -b bufferSize -s sizeOfBloom -i input_dir");
      return 0;
  }
  int monitor_size, bufferSize; 
  monitor_size = atoi(argv[2]);
  bufferSize = atoi(argv[4]);
  int bf_size = atoi(argv[6]);
  
  if(bf_size>1000)
    bf_size=1000;
  
  char *files_path =  malloc((strlen(argv[8])+1)*sizeof(char));
  strcpy(files_path,argv[8]);
  files_path[strlen(argv[8])]='\0';
  int p=0;
  char *command;
  int s_height = 10;
  char *line=NULL;
  size_t len = 0;
  ssize_t read;
  
  DIR *input_dir = opendir(files_path);
  char cp[258];
  //this will be a list of lists where every node will be a virus and will contain a list of the bloom filters that 
  //are about this virus
  List B_filters = List_create();
  //List that holds the virus stats for every country
  List viruses_travel_stats = List_create();
  bloom_filter *m_bf = Create(k_hashes,_bf_size);
  // Structures to store worker info
  struct travel_monitors *tmonitor_info;  // Stores worker pid & pipe ends
  tmonitor_info = calloc(monitor_size, sizeof (struct travel_monitors));

  init_monitors(tmonitor_info, monitor_size, bufferSize, files_path,k_hashes,_bf_size,10);
  assign_countries(tmonitor_info, monitor_size, input_dir, bufferSize);

  int available_monitors = 0;  // # of monitors ready to receive commands
  int total = 0;   
  int ccode;

  //for select()
  fd_set _read; 

  signals_unblock();  // Finished the setup

  while (1)
  {
    if (available_monitors != monitor_size)  // Wait for response from monitor
    {
      FD_ZERO(&_read);
      for (int i = 0; i < monitor_size; ++i)
        FD_SET(tmonitor_info[i].read_fd, &_read);  // Init read pipes' ends

      int check = select(FD_SETSIZE, &_read, NULL, NULL, NULL);
      if (check == -1){ 
        perror("select");
        continue;
      }
      for (int i = 0; i < monitor_size; ++i)
      {  
        if (FD_ISSET(tmonitor_info[i].read_fd, &_read))  // Find the pipe that has data to read from
        {
          char *msg = read_from_pipe(tmonitor_info[i].read_fd, bufferSize,m_bf);
          int opcode = get_opcode(msg);  // Operation code
          
          char *sanitised = malloc((msg_len_rd +1)*sizeof(char));     // Decoded message
          strcpy(sanitised, get_pipe_message(msg));
          sanitised[msg_len_rd] = '\0';
          free(msg);
          int flg = get_msg_from_monitor(opcode, sanitised, &total, &available_monitors,B_filters,viruses_travel_stats,m_bf);
          //this is for the addvaccinationquery 
          if(flg == COUNTRY_CHECK){
            for(int i=0; i<monitor_size; i++){
              ListNode found = ListFind(tmonitor_info[i].countries,sanitised,2);
              if(found!=NULL){
                kill(tmonitor_info[i].w_pid,SIGUSR1);
                break;
              }
            } 
          }
          free(sanitised);
        }
      }
    }
    else   // Every monitor is ready to receive requests
    {
      int len = 258;
      char line[len];
      signals_unblock();
      signals_block();
      signals_check(m_bf,B_filters,viruses_travel_stats);
      signals_unblock();
      //couldn't use getline as with the 1st project, as it didnt work great with the signals 
      if (fgets(line, len, stdin) == NULL)  // Get user input  
        continue;
      
      signals_block();  // Block signals while processing commands.
      //trim(line);
      
      strcpy(cp,line);
      command  = strtok(line," ");
      ccode = find_command(command);
      
      //exit
      if (get_command(cp,line, ccode, &available_monitors, &total, monitor_size, tmonitor_info, bufferSize,B_filters,viruses_travel_stats,m_bf,input_dir) == 1){
        free(files_path);
        break;  
      }  
    }
  }

  exit(0);
}
